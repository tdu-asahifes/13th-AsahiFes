function fetchAndUpdateData() {
  var now = new Date();
  const cutoffDate = new Date('2024-11-03T15:00:00+09:00');// 2024/11/3の15時

  // 15時以降の場合は結果を消してメッセージのみ表示
  if (now >= cutoffDate) {
    document.getElementById('results').innerHTML = '<p>後夜祭で結果発表を行うため結果の表示を一時中断しています。<br>集計は続いているので最後まで投票を続けてください！</p>';
    return;
  }
